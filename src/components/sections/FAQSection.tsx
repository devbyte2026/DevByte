...
export function FAQSection() {
  ...
  return (
    <section id="preguntas-frecuentes" className="py-16 md:py-24 px-4 md:px-8 bg-surface relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto relative">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <ScrollReveal key={index} delay={(index % 4) * 100 as 0 | 100 | 200 | 300}>
              <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md hover:shadow-accent/10 transition-all duration-300 border border-gray-100 hover:border-accent/30 group">
                ...
                >
                  <span className="font-semibold text-primary pr-4 group-hover:text-accent transition-colors duration-300">{faq.question}</span>
                  <ChevronDown
                    className={cn(
                ...
                >
                  <p className="px-6 pb-6 text-text-muted leading-relaxed">{faq.answer}</p>
                </div>
              </div>
            </ScrollReveal>
          ))}
